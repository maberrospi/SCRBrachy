...
def find_non_TN(model, pat_dir_ct, pat_dir_mask, device, threshold=0.5):
    ...
    for index, files in enumerate(tqdm(zip(cts_sorted, masks_sorted), desc='Slice')):
        assert os.path.exists(files[0]) or os.path.exists(files[1]), "One of the files does not exist"
...
def main():
    ...
    # Change between unet and attunet appropriately
    model.load_state_dict(
        torch.load(unet_model, map_location=device))

    # # Run inference on a single image
    path = "/home/ERASMUSMC/099035/Documents/MasksV2/MASKS14/mask14_2.png"
    path2 = "/home/ERASMUSMC/099035/Documents/CTimagesV2/CT14/ct14_2.npy"
    assert os.path.exists(path) or os.path.exists(path2), "One of the files does not exist"
    mask = Image.open(path)
    # img = Image.open(path2)
    img = np.load(path2)
    ...
    # ct_dir = "/home/ERASMUSMC/099035/Documents/CTimagesV2/CT14"
    # mask_dir = "/home/ERASMUSMC/099035/Documents/MasksV2/MASKS14"
